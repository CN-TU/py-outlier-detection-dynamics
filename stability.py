...
def predict_proba(X, train_scores, test_scores, method='linear'):
    ...
    if method == 'linear':
        ...
        pre_erf_score = (test_scores - np.mean(train_scores)) / (np.std(train_scores) * np.sqrt(2))
...
def stability_measure(Xtr, Xte, model, gamma, te_scores,
    ...
    for i in range(iterations):
        ...
        if hasattr(model, 'predict_proba') and callable(getattr(model, 'predict_proba')):
            try:    
                ...
                probs = np.ones(Xte.shape)*0.5
        else:
            tr_scores = model.predict(Xte)
            probs = predict_proba(Xte, tr_scores, te_scores, method='unify')
